...
class TestClientRetryConfiguration:
    ...
    def test_https_retry_adapter_reflects_constants(self) -> None:
        ...
        """
        with JuniperDataClient(base_url="https://api.example.com:8100", retries=2) as client:
...
class TestRetryBackoffJitter:
    ...
    def test_retry_adapter_carries_the_jitter(self) -> None:
        with JuniperDataClient(base_url="http://localhost:8100") as client:
            adapter = client.session.get_adapter("http://localhost:8100/")
    ...
    def test_backoff_schedule_actually_varies(self) -> None:
        ...
        with JuniperDataClient(base_url="http://localhost:8100", retries=5) as client:
            retry = client.session.get_adapter("http://localhost:8100/").max_retries
