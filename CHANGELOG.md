...
- **Exception context survives `pickle` and `copy`.** `BaseException.__reduce__` rebuilds from `args`, which holds only the message, so a round-trip would have returned an exception that looked correct and had silently dropped the new fields — the failure mode flake8-bugbear's `B042` warns about. A `__reduce__` override restores the full state; exceptions cross process boundaries here whenever a worker returns a failure to its parent.
